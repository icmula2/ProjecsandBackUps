...
def sentence1() -> Expr:
    ...
    # B = Expr('B')
    # C = Expr('C')
    AorB  = Expr('|', Expr('A'), Expr('B'))
    notAiffNotBoC = Expr('<=>', Expr('~', Expr('A')), Expr('|', Expr('~', Expr('B')), Expr('C')))
    notAornotBorC = Expr('|', Expr('~', Expr('A')), Expr('~', Expr('B')), Expr('C'))

    return Expr('&',  AorB, notAiffNotBoC, notAornotBorC)
    # return AorB & notAiffNotBoC & notAornotBorC
...
def sentence2() -> Expr:
    ...
    aImpNotBaNotD = Expr(">>", A, (~B & ~D))
    notBnotCImpA = Expr(">>", ~(B & ~C), A)
    cIfBoD = Expr("<=>", C, (B | D))
    notDImpC = Expr(">>", ~D, C)

    return Expr('&', cIfBoD, aImpNotBaNotD, notBnotCImpA, notDImpC)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    P1 = logic.PropSymbolExpr('PacmanAlive_1')
    P0 = logic.PropSymbolExpr('PacmanAlive_0')
    PB0 = logic.PropSymbolExpr('PacmanBorn_0')
    PK0 = logic.PropSymbolExpr('PacmanKilled_0')

    isAlive = P1 % disjoin((P0 & ~PK0), (~P0 & PB0))
    notAliveandBorn = ~(P0 & PB0)
    bornT0 = PB0

    return logic.conjoin(isAlive, notAliveandBorn, bornT0)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return not findModel(~(conclusion) & premise)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    finalConj = [] 
    for perm in itertools.combinations(literals, 2):
        finalConj.append(disjoin(~perm[0], ~perm[1]))
    return conjoin(finalConj)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacStrSymb = PropSymbolExpr(pacman_str, x, y, time=now)
    return pacStrSymb % disjoin(possible_causes)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    loc = [] 
    for d in DIRECTIONS: 
        prop = PropSymbolExpr(d, time = t)
        dir.append(prop)
    
    for x,y in all_coords: 
        imp = PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t))
        pacphysics_sentences.append(imp)
    
    for x,y in non_outer_wall_coords: 
        imp = PropSymbolExpr(pacman_str, x, y, time=t)
        loc.append(imp) 

    pacphysics_sentences.append(exactlyOne(loc))
    pacphysics_sentences.append(exactlyOne(dir))

    if (sensorModel != None )and (sensorModel(t, non_outer_wall_coords) != None): 
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    
    if (t!= 0) and (successorAxioms != None) and (successorAxioms(t, walls_grid, non_outer_wall_coords) != None):
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    atT0 = PropSymbolExpr(pacman_str,x0, y0, time = 0)
    KB.append(atT0)
    for t in range(50):
        print(t, "is the timestep")

        loc = []
        dir = []

        for x,y in non_wall_coords: 
            appendable = PropSymbolExpr(pacman_str, x, y, time = t)
            loc.append(appendable)
        
        for d in DIRECTIONS:
            appendable2 = PropSymbolExpr(d, time = t) 
            dir.append(appendable2)

        if t != 0:
            for x,y in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(x, y, t, walls_grid))

        KB.append(exactlyOne(loc))
        KB.append(exactlyOne(dir))


        m = findModel(PropSymbolExpr(pacman_str,xg, yg, time = t) & conjoin(KB))
        if m:
            return extractActionSequence(m, actions)        
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    for x,y in food:
        foodHere = PropSymbolExpr(food_str, x, y, time = 0)
        KB.append(foodHere)
    atT0 = PropSymbolExpr(pacman_str, x0, y0, time = 0)
    KB.append(atT0)
    for t in range(50):
        loc, dir, eaten = [],[],[]
        for x,y in non_wall_coords:
            expr = PropSymbolExpr(pacman_str, x, y, time = t)
            loc.append(expr)
        for x,y in food: 
            foodPos = ~PropSymbolExpr(food_str, x, y, time = t)
            eaten.append(foodPos)
        for d in DIRECTIONS:
            expr = PropSymbolExpr(d, time = t)
            dir.append(expr)
        
        for x,y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, t+1, walls))
        for x,y in food:
            atFood = PropSymbolExpr(pacman_str, x, y, time = t) & PropSymbolExpr(food_str, x, y, time = t)
            foodAte = ~PropSymbolExpr(food_str, x, y, time = t+1)
            foodEaten = foodAte % atFood
            noFood = ~PropSymbolExpr(food_str, x, y, time = t)
            KB.append(noFood | foodEaten)

        KB.append(exactlyOne(loc))
        KB.append(exactlyOne(dir))

        n = findModel(conjoin(eaten) & conjoin(KB))
        if n: 
            return extractActionSequence(n, actions)
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x,y in non_outer_wall_coords:
            ...
            isThere = findModel(pos& conjoin(KB))
            notThere = findModel(~pos & conjoin(KB))
            if isThere: 
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            notWallSt = ~PropSymbolExpr(wall_str, x, y)
            wallSt = PropSymbolExpr(wall_str, x, y)
            if entails(conjoin(KB), notWallSt):
                known_map[x][y] = 0
                KB.append(~PropSymbolExpr(wall_str, x, y))
            elif entails(conjoin(KB), wallSt):
                known_map[x][y] = 1 
                KB.append(PropSymbolExpr(wall_str, x, y))
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            notPacman = ~(PropSymbolExpr(pacman_str, x, y, time = t))
            pacmanSt = PropSymbolExpr(pacman_str, x, y, time = t)
            notWallSt  = ~(PropSymbolExpr(wall_str, x, y))
            wallSt = PropSymbolExpr(wall_str, x, y)

            if entails(conjoin(KB), notPacman):
                KB.append(notPacman)
            elif entails(conjoin(KB), pacmanSt):
                possible_locations.append((x,y))
                KB.append(pacmanSt)
            else: 
                possible_locations.append((x,y))
