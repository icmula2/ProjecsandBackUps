...
class Move {
    ...
    boolean isExtend() {
        if (Math.abs(row1() - row0()) == 0 && Math.abs(col1() - col0()) == 0) {
            return false;
        } else if (Math.abs(row1() - row0()) <= 1) {
            if (Math.abs(col1() - col0()) <= 1) {
                return true;
            }
        }
        return false;
    ...
    boolean isJump() {
        int rowTravel = Math.abs(row1() - row0());
        int colTravel = Math.abs(col1() - col0());
        if (!(rowTravel <= 1 && colTravel <= 1)) {
            return true;
        }
        return false;
    }
