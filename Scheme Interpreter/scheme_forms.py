...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        if type(signature) == Pair:
            #procedure
            env.define(signature.first, scheme_eval(expressions.rest, env))
            return func_name
        else:
            #variable
            ...
            return signature
        """
        if expressions.first is not "(":
            return expressions.first
        else:
            return expression.rest.first
        """
        # END PROBLEM 4
        ...
        lambda_procedure = do_lambda_form(Pair(signature.rest, expressions.rest), env)
